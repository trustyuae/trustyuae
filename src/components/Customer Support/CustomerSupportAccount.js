...
/** Account orders API may return `items` or grouped `items_by_category`. */
...
/** Top-level `note` or aggregated `note` from line items under `items_by_category`. */
function getAccountOrderNoteText(order) {
  if (!order) return "";
  if (order.note != null && String(order.note).trim() !== "") {
...
function CustomerSupportAccount() {
  ...
  const accountPagination = useSelector(
    (state) => state?.customerSupport?.accountOrdersPagination
  );

  useEffect(() => {
    const rows = accountOrdersRaw.map((o, i) => ({
      ...o,
      id: o.order_id ?? i,
      date_display: o.order_date
        ? dayjs(o.order_date).format("YYYY-MM-DD")
        : "",
      order_status: formatAccountOrderStatus(o.items_by_category),
    }));
    setOrders(rows);
  }, [accountOrdersRaw]);
  ...
  useEffect(() => {
    if (accountPagination) {
      setOverAllData({
        total_count: accountPagination.total_orders ?? 0,
        total_pages: accountPagination.total_pages ?? 1,
      });
      setTotalPages(accountPagination.total_pages ?? 1);
    }
  }, [accountPagination]);
  ...
  function buildAccountApiUrl() {
    ...
    let url = `wp-json/custom-account-orders/v1/account-orders/?page=${page}&per_page=${pageSize}`;
    if (orderId) {
  ...
  const handleOpenNoteModal = (row) => {
    const text = getAccountOrderNoteText(row);
      ...
      renderCell: (value) => {
        return (
          <Box className="d-flex justify-content-center align-items-center">
            <Link
              to={`/order_details/${value?.row?.order_id}?cs=1&account=1`}
              className=" d-flex "
            >
              <Button
                type="button"
  ...
  const handleChange = (event, value) => {
    setPage(value);
    dispatch(
      setCurrentPage({ tableId: "CustomerSupportAccount", page: value })
    );
  };
  ...
  const runAccountSearch = () => {
    ...
    setSearchOrderID(v);
    setPage(1);
    let url = `wp-json/custom-account-orders/v1/account-orders/?page=1&per_page=${pageSize}`;
...
export default CustomerSupportAccount;
